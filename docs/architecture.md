...
Launcher 私有的 `desktopRuntime`、`desktopPnpmBootstrap`、Electron executable、Node helper 和 ABI 环境不是第三方 API。Beta 分支的公开 contract 只有 `dsh-plugin-desktop-beta/profile-service` 与 `dsh-plugin-desktop-beta/pnpm`。
